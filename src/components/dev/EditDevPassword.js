...
const EditDevPassword = ({ user, editPassword, setEditPassword, editMenu, toggleEditMenu, setData }) => {
    ...
    const handleChange = (e) => {
        setEditPassword((prev) => ({
            ...prev,
            [e.target.name]: e.target.value || e.target.options.selectedIndex
        }));
    };
    ...
    const handleClick = async (e) => {
        ...
        const res = await axios.patch(
            `${process.env.REACT_APP_API}/devs/${editPassword._id}/${user._id}`, {
            ...
            type: editPassword.type,
        })
        setData(res)
        toggleEditMenu('close')
    };
        ...
        <div className={`lg:block relative z-50 ${(editMenu === 'hidden' ? 'hidden' : '')}`}>
            <nav className={`fixed top-0 right-0 bottom-0 flex flex-col w-3/4 lg:w-96 sm:max-w-md pt-6 pb-8 bg-white dark:bg-gray-800 border-l dark:border-gray-700 overflow-y-auto ${(editMenu === 'hidden' ? 'hidden' : '')}`}>
                <div className="px-4 pb-6">
                    ...
                    <div className="mb-6">
                            <label className="block text-sm dark:text-white font-medium mb-2" htmlFor="editName">Name</label>
                        ...
                        <div className="mb-6">
                            <label className="block text-sm dark:text-white font-medium mb-2" htmlFor="editURL">URL</label>
                            <input disabled={disabledInput}
                                className="w-full px-4 py-3 mb-2 text-sm disabled:bg-indigo-50 dark:disabled:bg-lightGray-900 dark:disabled:text-indigo-0 placeholder-gray-500 dark:placeholder-gray-600 bg-white dark:bg-gray-900 dark:text-white border rounded"
                                type="text" name="url" placeholder="https://www.devflow.com" id="editURL" value={editPassword.url} onChange={handleChange} />
                        </div>
                        ...
                        <div className="mb-6">
                            ...
                            <div className="relative">
                                <select disabled={disabledInput}
                                    className="appearance-none block w-full px-4 py-3 mb-2 text-sm disabled:bg-indigo-50 dark:disabled:bg-lightGray-900 dark:disabled:text-indigo-0 bg-white dark:bg-gray-900 dark:text-white border rounded"
                                    name="type" id="editType" onChange={handleChange} value={editPassword.type}>
...
export default EditDevPassword