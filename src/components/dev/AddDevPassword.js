...
const AddDevPassword = ({ user, addMenu, setData, toggleAddMenu }) => {
    ...
    const handleChange = (e) => {
        setNewPassword((prev) => ({
            ...prev,
            [e.target.id]: e.target.value || e.target.options.selectedIndex
        }));
    };
    const handleClick = async (e) => {
        e.preventDefault();
        await axios.post(`${process.env.REACT_APP_API}/devs/${user._id}`, newPassword)
        setData(newPassword)
    };
    ...
    return (
        <div className={`${(addMenu === 'hidden' ? 'hidden' : '')} lg:block relative z-50`}>
            <nav className={`fixed top-0 right-0 bottom-0 flex flex-col w-3/4 lg:w-96 sm:max-w-md pt-6 pb-8 bg-white dark:bg-gray-800 border-l dark:border-gray-700 overflow-y-auto ${(addMenu === 'hidden' ? 'hidden' : '')}`}>
                <div className="px-4 pb-6">
                    <form>

                        <div className="mb-6">
                            <label className="block text-sm dark:text-white font-medium mb-2" htmlFor="url">Name</label>
                            <input
                                ...
                                type="text" name="name" placeholder="Discord" id="name" value={newPassword.name} onChange={handleChange} />
                        </div>
                        ...
                        <div className="mb-6">
                            ...
                            <input
                                className="block w-full px-4 py-3 mb-2 text-sm placeholder-gray-500 dark:placeholder-gray-600 bg-white dark:bg-gray-900 dark:text-white border rounded"
                                type="text" name="url" placeholder="https://webflow.com" id="url" value={newPassword.url} onChange={handleChange} />
                        </div>
                        ...
                        <div className="mb-6">
                            ...
                            <textarea
                                className="block w-full px-4 py-3 mb-2 text-sm placeholder-gray-500 dark:placeholder-gray-600 bg-white dark:bg-gray-900 dark:text-white border rounded"
                                name="notes" rows="5" placeholder="Write something..." id="notes" value={newPassword.notes} onChange={handleChange} ></textarea>
                        </div>

                        <div className="mb-6">
                            <label className="block text-sm dark:text-white font-medium mb-2" htmlFor="type">Type</label>
                            <div className="relative">
                                <select
                                    className="appearance-none block w-full px-4 py-3 mb-2 text-sm placeholder-gray-500 dark:placeholder-gray-600 bg-white dark:bg-gray-900 dark:text-white border rounded"
                                    name="type" id="type" onChange={handleChange} value={newPassword.type}>
                        ...
                        <div className="flex text-center w-full mb-2">
                            ...
                            <button onClick={toggleAddMenu}
                                className="inline-block w-1/2 px-6 py-3 ml-2 font-medium text-white bg-indigo-500 hover:bg-indigo-600 rounded transition duration-200"
