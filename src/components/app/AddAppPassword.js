...
const AddAppPassword = ({ user, addMenu, setData, toggleAddMenu }) => {
    ...
    const handleClick = async (e) => {
        e.preventDefault();
        await axios.post(`${process.env.REACT_APP_API}/apps/${user._id}`, newPassword)
        setData(newPassword)
    };
        ...
        <div className={`${(addMenu === 'hidden' ? 'hidden' : '')} lg:block relative z-50`}>
            <nav
                ...
                <div className="px-4 pb-6">
                    <form>
                        <div className="mb-6">
                            <label className="block text-sm dark:text-white font-medium mb-2" htmlFor="name">App Name</label>
                            <input
